...
// input money can be positive(add) or negative(reduce)
func (t *SimpleChaincode) changeMoney(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	if len(args) != 2 {
		return nil, errors.New("Incorrect number of arguments. Expecting 2")
	}
	var A string    // Entities
	...
	// Balance can't lower than -100
	if Aval < -100 {
		return nil, errors.New("Run out of money")
	}
